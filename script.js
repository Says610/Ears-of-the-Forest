...
const hazardMat = new THREE.MeshStandardMaterial({color:0x553322});
for(let i=0;i<25;i++){
  const log = new THREE.Mesh(new THREE.CylinderGeometry(0.2,0.3,4), hazardMat);
  log.position.set(Math.random()*terrainSize - terrainSize/2,0.1,Math.random()*terrainSize - terrainSize/2);
...
for(let i=0;i<250;i++){
  ...
  trunk.position.set(x,2,z); leaves.position.set(x,6,z);
...
function spawnClassmate(x,z){
  ...
  scene.add(mesh);
  classmates.push({mesh,following:true});
}
function updateClassmates(){ classmates.forEach(c=>{
  if(c.following) c.mesh.position.lerp(camera.position,0.002);
}); }

// ==========================================
// WOLF CLASS (animated + attack + patrol + fear)
class Wolf{
  constructor(x,z,boss=false,cave=false){
    this.loader = new THREE.GLTFLoader();
    this.boss=boss; this.health=boss?400:60; this.cave=cave; this.state="stalking";
    this.fear=0; this.mesh=null; this.mixer=null;
  ...
  update(delta){
    ...
    switch(this.state){
      case "stalking":
        if(dist<20) this.state="circling";
        this.mesh.position.lerp(camera.position,0.001);
        break;
      case "circling":
        this.mesh.position.x += Math.sin(Date.now()*0.002)*0.03;
        this.mesh.position.z += Math.cos(Date.now()*0.002)*0.03;
        if(dist<6) this.state="chasing";
        break;
      case "chasing":
        this.mesh.position.lerp(camera.position,0.006);
        if(dist<2 && this.attackCooldown<=0){
          takeDamage(this.boss?30:10);
...
const storyFlags = {shortcut:false,helpedFriend:false,enteredCave:false,fearful:false};
...
function startStory(){
  speak("You wake up at a sleepover, excited for the forest trip!",[
    {text:"Get ready",action:()=>choosePath()}
  ]);
}

function choosePath(){
  speak("Do you take the safe path or the shortcut through dense forest?",[
...
function enterForest(){
  ...
  speak("Your friends are with you. You hear rustling in the forest...",[
...
function timedWolves(){
  const interval = setInterval(()=>{
    ...
    if(t>600 && !eventsTriggered.horde){ for(let i=0;i<8;i++) spawnWolf(Math.random()*40-20,Math.random()*40-20); eventsTriggered.horde=true; howlAudio.play(); clearInterval(interval); }
  },1000);
}
...
function triggerEnding(type){
  ...
  let t=0; ending.innerText = type.toUpperCase()+" ENDING";
  ending.style.display="block";
  function animateCut(){
    t++;
    camera.position.y += 0.01;
    if(t<200) requestAnimationFrame(animateCut);
  }
  animateCut();
}
